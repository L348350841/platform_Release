...
# from backend.app.run import BACKEND_ENV
# if BACKEND_ENV == 'development':
#     from app.POSIXC import noticeToC,received,dataFromC

# from app.POSIXC import noticeToC,received,dataFromC,data_length
...
def getFreshiftData():
    ...
    try:
        ...
        if time in [0, None]:
            ...
            target = Freshift.query.filter(Freshift.freshift_time == max_time).first()
            if target:
            ...
            print(gettime_str)
            print('gettime_str')
            target = Freshift.query.filter(Freshift.freshift_time == gettime).first()
            if target:
                str_numbers = target.value_b
                str_numbers_list = str_numbers.split(',')
                # out1 = np.array([float(num.strip()) for num in str_numbers_list])
...
def addParaValue():
    ...
    try:
        ...
        processed_data = {
            ...
            'writable': False if data.get('writable') == '0' else True,
            'unit': data.get('unit'),
            'read_value': float(data.get('write_value')) if 'write_value' in data else None,
...
def startCollect(): 
    # 从 JSON 请求体中获取参数
    data = request.json
    params = data.get('arg')  # 获取名为 'arg' 的参数
    print(f"接收到参数: params = {params}")  # 打印参数，用于调试
    ...
    # ]
    print(params)
    # noticeToC(params,delta_f) # 需要二项式拟合
    # noticeToC(params)
...
def checkCollection(): 
    ...
    if receivedC == True:
        ...
        # 模拟二维数据10*10
        # rows, cols = 10, 10
        # data = [[random.randint(-100, 100) for _ in range(cols)] for _ in range(rows)]

        # 动态设置数组的元素个数
        n =  500 # 你可以根据需要更改这个值 data_length
        # 生成一维数组，包含 n 个随机整数，范围在 -100 到 100 之间
        data = [random.randint(10, 100) for _ in range(n)]
    # 获取当前时间戳
    freshift_time = datetime.now()
    # 使用 ', '.join(map(str, data)) 来生成字符串
    value_b_str = ', '.join(map(str, data))
    # 创建一个新的 Temp 记录
    new_record = Freshift(freshift_time=freshift_time, value_b=value_b_str)
    # 添加到数据库会话
    db.session.add(new_record)
    # 提交事务
    db.session.commit()
  
    return jsonify({'completed': completed, "data": data})
